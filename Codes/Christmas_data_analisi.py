...
fileNames = ll.remove_extension(ll.get_file_names(file_path + "data_gennaio_2/"))
...
for fileName in fileNames:
    data = ll.read_histogram_data(file_path + "data_gennaio_2/" + fileName + ".Spe")
    ...
    mpr.plot_hist_MPL(hist, file_path + "plots/hist/h_gennaio_2_" + time + ".png")
    fit_result = fit_photopeak(hist, file_path + "plots/fit/h_fit_gennaio_2_" + time + ".png", noise_threshold=0.5, n_peaks=2)

    RE.append(fit_result[0])
    E_mean.append(fit_result[1])
    times.append(time)

with open("Misure_gennaio_2.txt", "w") as file:
