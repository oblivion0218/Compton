...
for fileName in fileNames:
    ...
    mpr.plot_hist_MPL(hist, file_path + "plots/hist/h_" + time + ".png")
    fit_result = fit_photopeak(hist, file_path + "plots/fit/h_fit" + time + ".png", noise_threshold=0.5, n_peaks=2)

    RE.append(fit_result[0])
    E_mean.append(fit_result[1])
    times.append(time)

with open("Misure_invernali.txt", "w") as file:
